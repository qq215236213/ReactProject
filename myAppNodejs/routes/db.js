//https://www.npmjs.com/package/mssql#commit-callback
//导入mssql模块
 ...
 * @param {string} sqltext 执行的sql语句
 * @param {JSON} params sql语句中的参数
 * @param {function} func 回调函数 共有一个参数 error:错误消息 
 */
sql.execTransaction = function(sqltext,params,func){
    var transaction = new mssql.Transaction(new mssql.ConnectionPool(config,err =>{
        if(err){
            func(err);
            return;
        }
        transaction.begin(err1 =>{
            ...
            request.query(sqltext, (err2,result) =>{
                if(err2){
                    func(err2);
                    if(!rolledBack){
                        transaction.rollback(err3 =>{
                            func(err3);
                            return;
                        })
                    }
