...
class ShareRMB extends Component{
	constructor(props){
		super(props);
		this.state = {
			datalist: []
	...
	componentDidMount(){
		$.get('http://www.jzg.com/share/info',{memberid:cookie.Cookie('memberid'),accesstoken:cookie.Cookie('token'),state:1},(d) =>{
			if(d.IsError){
				show.msg(d.Msg);
				return;
			}
